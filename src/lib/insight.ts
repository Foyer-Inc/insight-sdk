...
export class Insight {
  ...
  public baseURL = 'https://api-v2.foyer.ai';
  ...
  private async classifyRequest(
    ...
    let payload: ClassifyPayload = {
      ...
      includeSegmentations:
        options.includeSegmentations ?? this.includeSegmentations,
      includeTagpoints: options.includeTagpoints ?? this.includeTagpoints,
      detectionsRequested:
        options.detectionsRequested ?? this.detectionsRequested,
    };

    payload = await addImagesToPayload(images, payload);

    let response = await fetch(`${this.baseURL}/Media/classify`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    if (Array.isArray(result)) {
      return result.map(
        (r: ClassifyResponse, idx: number) => new ClassifyResult(images[idx], r)
      );
    } else {
